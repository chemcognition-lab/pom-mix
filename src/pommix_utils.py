...
def get_embeddings_from_smiles(
    smi_list: list[str], file_path: str, gnn: Optional[torch.nn.Module] = None
):
    ...
    if gnn is None:
        gnn = GraphNets.from_json(
            node_dim=ds.node_dim,
            edge_dim=ds.edge_dim,
            json_path=f"{file_path}/hparams.json",
        )
        state_dict = torch.load(
            f"{file_path}/gnn_embedder.pt", map_location=torch.device("cpu")
        )
...
def create_tvt_split_indices(
    ...
    train_ind, train_strat, test_ind, _ = train_test_split(
        indices, stratify, test_size=test_size, stratify=stratify, random_state=seed
    )
