...
if __name__ == "__main__":
    ...
    for id, train, val, test in sl.load_splits(dl.features, dl.labels):
        ...
        if augment:
            train_features, train_labels = dl.single_molecule_mixture_gslf_jaccards(
            ...
            with torch.no_grad():
                ...
                graph_list, train_indices = get_mixture_smiles(
                    train_features, from_smiles
                )
                train_gr = Batch.from_data_list(graph_list).to(device)
                train_features = (
                    embedder.graphs_to_mixtures(train_gr, train_indices, device=device)
                    .detach()
                    .cpu()
                    .numpy()
                )
                ...
                val_features = (
                    embedder.graphs_to_mixtures(val_gr, val_indices, device=device)
                    .detach()
                    .cpu()
                    .numpy()
                )

                graph_list, test_indices = get_mixture_smiles(
                    test_features, from_smiles
                )
                test_gr = Batch.from_data_list(graph_list).to(device)
                test_features = (
                    embedder.graphs_to_mixtures(test_gr, test_indices, device=device)
                    .detach()
                    .cpu()
                    .numpy()
                )
        ...
        train_loader = DataLoader(
            TensorDataset(train_features, y_train), batch_size=batch_size, shuffle=True
        )
