...
def create_k_molecules_split(
    ...
    # get mixture lengths
    if geometric:
        mixture_lengths = np.array(
            [np.sqrt(np.prod(calculate_inner_lengths(row))) for row in features]
        ).reshape(-1, 1)
    else:
...
def create_molecule_identity_splits(
    mixture_smiles: np.ndarray,
    num_splits: Optional[int] = 5,
    valid_percent: Optional[float] = 0.1,
) -> Tuple[list[int], list[int], list[int]]:
    """
    ...
    mixture_smiles = np.array(
        [np.concatenate(item) for item in mixture_smiles], dtype=object
    )
    ...
    for i, smi in enumerate(sorted_strings):
        inds = [
            ...
            if any(item == smi for item in sublist)
        ]
...
if __name__ == "__main__":
    ...
    for i, (train_idx, test_idx) in enumerate(
        splits.split(list(range(len(df))), dataset_id)
    ):
        # do a split for 70/10/20 train/validation/test
        train_idx, valid_idx = train_test_split(
            train_idx, train_size=train_frac, random_state=seed
        )
        np.savez(
            OUTPUT_DIR / f"random_cv{i}.npz",
            identifier=f"cv{i}",
            training=train_idx,
    ...
    for k in [7, 8, 9, 10, 13, 18, 20, 27, 30, 40]:
        train_idx, valid_idx, test_idx = create_k_molecules_split(
            dl.features, k=k, geometric=True
        )
        print(f"For k {k}: {len(train_idx) / len(dl.features)}")
        np.savez(
            OUTPUT_DIR / f"ablate_components{k}.npz",
            identifier=f"k{k}",
            training=train_idx,
            validation=valid_idx,
            testing=test_idx,
    ...
    for i, (train_idx, valid_idx, test_idx) in enumerate(all_splits):
        np.savez(
            OUTPUT_DIR / f"ablate_molecules{i}.npz",
            identifier=f"m{i}",
            training=train_idx,
            validation=valid_idx,
            testing=test_idx,
    ...
    all_splits, removed_smiles = create_lso_molecule_identity_splits(
        dl.features, num_splits=5
    )
    print(f"Here are the smiles that are only found in test set: {removed_smiles}")
    for i, (train_idx, valid_idx, test_idx) in enumerate(all_splits):
        np.savez(
            OUTPUT_DIR / f"lso_molecules{i}.npz",
            identifier=f"lso{i}",
            training=train_idx,
            validation=valid_idx,
            testing=test_idx,
